 ...
 * deposit_paid flag and writes a row into the payments ledger. No manual
 * "mark as paid" needed — the desk updates itself.
 *
 ...
 *   4. Stripe → Developers → Webhooks → "Add endpoint" → paste the URL →
 *      event: `checkout.session.completed` → Add → copy the "Signing secret" (whsec_…)
 *   5. Edge Functions → Secrets → add:
 *        STRIPE_SECRET_KEY   = sk_test_… (same key as create-checkout)
