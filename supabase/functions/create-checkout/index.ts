 ...
 *
 * Creates a Stripe Checkout session for a booking's 30% deposit. The amount
 * is computed HERE (server-side) from the package price in site_content, so
 * clients can never tamper with the price — they only pass their booking ref.
 *
 * SETUP (no CLI needed — via the Supabase dashboard):
 *   1. Edge Functions → "New function" → name it `create-checkout` → paste this file → Deploy
 *   2. On the function's page, turn OFF "Verify JWT" (clients call this before signing in)
 *   3. Edge Functions → Secrets → add:  STRIPE_SECRET_KEY = sk_test_… (or sk_live_…)
 *
 * Then in Stripe: Developers → API Keys → copy your secret key for step 3.
 * Use sk_test_ keys while testing (card 4242 4242 4242 4242, any future date, any CVC).
 */
...
Deno.serve(async (req) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: cancel_url || "https://imaginarycapture.netlify.app/#book",
    });
